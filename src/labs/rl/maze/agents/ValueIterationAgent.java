    ...
    extends StochasticAgent
{
    ...
    public static final double GAMMA = 1.2; // feel free to change this around!
    ...
    public void valueIteration(StateView state)
    {
        ...
        double delta;
        do {
            // delta to track max change in utility for any state in this iteration
            delta = 0;
            // newUtilities - a map to store updated utility values in this iteration
            Map<Coordinate, Double> newUtilities = new HashMap<>(utilities);

            // iterate over each state in the current utility map
            for (Coordinate c : utilities.keySet()) {
                if (!isTerminalState(c)) {  // skip terminal states as their utilities don't change
                    double maxUtility = Double.NEGATIVE_INFINITY;

                    for (Direction d : TransitionModel.CARDINAL_DIRECTIONS) {  // check each possible action from this state
                        ...
                        for (Pair<Coordinate, Double> transition : TransitionModel.getTransitionProbs(state, c, d)) {
                            ...
                            actionUtility += prob * (reward + GAMMA * utilities.get(nextState));  // expected utility
                        }
